...
MatrixXdr prevelance;
MatrixXdr liability_factor;
bool bin_pheno_flag;
...
void read_pheno2(int Nind, std::string filename){
	...
	pheno.resize(Nind, phenocount);
        ...
        if (bin_pheno_flag) {
                prevelance = MatrixXdr::Zero(phenocount, 1);
        }
	int i=0;  
        if (bin_pheno_flag) {
                while(std::getline(ifs, line)){
                ...
                for (int j = 0; j < phenocount; j++) {
                        prevelance(j, 0) = prevelance(j, 0)/Nind;
                }
                // cout << prevelance << endl;
...
MatrixXdr  compute_XXz (int num_snp,MatrixXdr Zvec, int num_vec){
        res.resize(num_snp, num_vec);

	// if(use_mailman==true)
	// 	multiply_y_pre_fast(Zvec,Nz,res, false);
	// else	
        // 	res=gen*Zvec;
        mm.multiply_y_pre(Zvec,num_vec,res, false);
	...
	for(int j=0; j<num_snp; j++)
            for(int k=0; k<num_vec;k++)
                res(j,k) = res(j,k)*stds(j,0);
        MatrixXdr resid(num_snp, num_vec);
	...
	for(int k=0; k<num_vec; k++)
       ...
       MatrixXdr new_res(num_vec, Nindv);
	
	// if(use_mailman==true)
	//     multiply_y_post_fast(new_zb, Nz, new_res, false);
	// else
	//     new_res=new_zb*gen;
        mm.multiply_y_post(new_zb, num_vec, new_res, false);
       MatrixXdr new_resid(num_vec, num_snp);
...
int main(int argc, char const *argv[]){
	...
	if(command_line_opts.exannot==true)
        ...
        filename=command_line_opts.PHENOTYPE_FILE_PATH;
        bin_pheno_flag = command_line_opts.bin_pheno_flag;
        read_pheno2(Nindv,filename);
        cout<<"Number of individuals: "<<Nindv<<endl;
        if (bin_pheno_flag) {
                liability_factor = MatrixXdr::Zero(phenocount, 1);
                boost::math::normal_distribution<> norm_dist(0,1);
                for (int j = 0; j < phenocount; j++) {
                        double temp = prevelance(j, 0);
                        temp = boost::math::quantile(norm_dist, 1-temp);
                        temp = boost::math::pdf(norm_dist, temp);
                        liability_factor(j, 0) = prevelance(j, 0)*(1-prevelance(j, 0))/(temp*temp);
        ...
        // std::string param_file=command_line_opts.simul_par_PATH;
        ...
        if(hetero_noise == true) {
                wt=MatrixXdr::Zero(Nindv,(Nbin+nongen_Nbin+Nenv+1)*phenocount);
                vt=MatrixXdr::Zero(Nindv,(Nbin+nongen_Nbin+Nenv+1)*(Nbin+nongen_Nbin+Nenv+1)*phenocount);
        } else {
                wt=MatrixXdr::Zero(Nindv,(Nbin+nongen_Nbin+1)*phenocount);
                vt=MatrixXdr::Zero(Nindv,(Nbin+nongen_Nbin+1)*(Nbin+nongen_Nbin+1)*phenocount);
        }
        ...
        for (int jack_index=0;jack_index<Njack;jack_index++){	
                ...
                for (int bin_index=0;bin_index<Nbin;bin_index++){
                        ...
                        if(num_snp!=0){
                                ...
                                output=compute_XXz(num_snp,all_zb, Nz);
                                ...
                                if (gen_by_env == true) {
                                        ...
                                        for (int env_index=0;env_index<Nenv;env_index++){
                                                ...
                                                output_env=compute_XXz(num_snp,env_all_zb, Nz);                                   
                                                ...
                                                if (both_side_cov==true){
                                                        ...
                                                        output_env=compute_XXz(num_snp,env_all_Uzb, Nz);
                                                        output_env=output_env.array().colwise()*Enviro.col(env_index).array();
                                                ...
                                                if (phenocount > Nz) {
                                                        ...
                                                        for (int k = 0; k < num_pheno_block; k++) {
                                                                ...
                                                                // analytic se
                                                                temp_xxy = compute_XXz(num_snp, cur_scaled_pheno, cur_num_pheno);
                                                                wt.block(0, gxe_bin_index*phenocount+tstart, Nindv, cur_num_pheno) += temp_xxy;
                                                        ...
                                                        // analytic se
                                                        temp_xxy = compute_XXz(num_snp, scaled_pheno, phenocount);
                                                        wt.block(0, gxe_bin_index*phenocount, Nindv, phenocount) += temp_xxy;
                                ...
                                if(both_side_cov==false){
                                        if (phenocount > Nz) {
                                                ...
                                                for (int k = 0; k < num_pheno_block; k++) {
                                                        ...
                                                        temp_xxy = compute_XXz(num_snp, cur_pheno, cur_num_pheno);
                                                        wt.block(0, bin_index*phenocount+tstart, Nindv, cur_num_pheno) += temp_xxy;
                                                ...
                                                temp_xxy = compute_XXz(num_snp, pheno, phenocount);
                                                wt.block(0, bin_index*phenocount, Nindv, phenocount) += temp_xxy;
                                        ...
                                        if (phenocount > Nz) {
                                                ...
                                                temp_xxy = compute_XXz(num_snp, new_pheno, phenocount);
                                                wt.block(0, bin_index*phenocount, Nindv, phenocount) += temp_xxy;
        ...
        cout<<"Finished reading and computing the first pass over all blocks"<<endl;
        cout << endl;
        cout << endl;
        ...
        if (hetero_noise == true) {
                ...
                for (int env_index=0;env_index<Nenv;env_index++){
                        ...
                        for (int i = 0; i < phenocount; i++) {
                                ...
                                wt.col(hetro_index*phenocount+i) = scaled_pheno.col(i).array()*Enviro.col(env_index).array();
                        }
                        len.push_back(1);
                } 
        }
        ...
        for (int k = 0; k < gen_Nbin+nongen_Nbin; k++) {
                for (int j = 0; j < phenocount; j++) {
                        wt.col(k*phenocount+j) = wt.col(k*phenocount+j)/len[k];
                }  
        }

        
        if(both_side_cov==true){
                wt.block(0, Nbin*phenocount, Nindv, phenocount) = new_pheno; 
        } else {
                wt.block(0, Nbin*phenocount, Nindv, phenocount) = pheno;
        }
        // read_simul_par(param_file);
                        ...
                        if(num_snp!=0){
                                ...
                                for (int k = 0; k < (total_Nbin+1); k++) { 
                                        if (phenocount > Nz) {
                                                ...
                                                int tstart = 0;
                                                int cur_num_pheno;
                                                for (int j = 0; j < num_pheno_block; j++) {
                                                        if ((j == num_pheno_block - 1) && (num_pheno_remain > 0))
                                                                cur_num_pheno = num_pheno_remain;
                                                        else
                                                                cur_num_pheno = Nz;
                                                        cur_wt = wt.block(0, k*phenocount+tstart, Nindv, cur_num_pheno);
                                                        val_temp = compute_XXz(num_snp, cur_wt, cur_num_pheno);
                                                        vt.block(0, bin_index*(total_Nbin+1)+k*phenocount+tstart, Nindv, cur_num_pheno) += val_temp/len[bin_index];
                                                        tstart += cur_num_pheno;
                                                }

                                        } else {
                                                cur_wt = wt.block(0, k*phenocount, Nindv, phenocount);
                                                val_temp = compute_XXz(num_snp, cur_wt, phenocount);
                                                vt.block(0, bin_index*(total_Nbin+1)+k*phenocount, Nindv, phenocount) += val_temp/len[bin_index];
                                        }
                                }
                                if (gen_by_env == true) {
                                        int gxe_bin_index;
                                        MatrixXdr scaled_vec;
                                        scaled_vec.resize(Nindv, (total_Nbin+1)*phenocount);
                                        for (int env_index=0;env_index<Nenv;env_index++){
                                                if(Annot_x_E==true)  
                                                        gxe_bin_index=total_Nbin+(env_index*total_Nbin)+bin_index;
                                                else
                                                        gxe_bin_index=total_Nbin+env_index;  

                                                for (int k = 0; k < (total_Nbin+1); k++) {
                                                        for (int j = 0; j < phenocount; j++) {
                                                                scaled_vec.col(k*phenocount+j) = wt.col(k*phenocount+j).array()*Enviro.col(env_index).array();
                                                        }
                                                }
                                                ...
                                                if (phenocount > Nz) {
                                                        ...
                                                        for (int k = 0; k < num_pheno_block; k++) {
                                                                if ((k == num_pheno_block - 1) && (num_pheno_remain > 0))
                                                                        ...
                                                                        cur_num_pheno = Nz;
                                                                for (int j = 0; j < (total_Nbin+1); j++) {
                                                                     cur_wt = scaled_vec.block(0, j*phenocount+tstart, Nindv, cur_num_pheno);  
                                                                     val_temp = compute_XXz(num_snp, cur_wt, cur_num_pheno); 
                                                                     vt.block(0, Nbin*(total_Nbin+1)+j*phenocount+tstart, Nindv, cur_num_pheno) += val_temp/len[bin_index];
                                                                }
                                                                tstart += cur_num_pheno;
                                                        }
                                                        ...
                                                        for (int j = 0; j < (total_Nbin+1); j++) {
                                                                cur_wt = scaled_vec.block(0, j*phenocount, Nindv, phenocount);  
                                                                val_temp = compute_XXz(num_snp, cur_wt, phenocount); 
                                                                vt.block(0, Nbin*(total_Nbin+1)+j*phenocount, Nindv, phenocount) += val_temp/len[bin_index];
                                                        }
                                                }
                                        }
                                }
        ...
        cout<<"Finished reading and computing the second pass over all blocks"<<endl;
        cout << endl;
        cout << endl;
        ...
        if (hetero_noise == true) {
                for (int env_index=0;env_index<Nenv;env_index++) {
                        ...
                        for (int k = 0; k < (total_Nbin+1); k++) {
                                for (int j = 0; j < phenocount; j++) {
                                        MatrixXdr temp_vec = wt.col(k*phenocount+j).array()*Enviro.col(env_index).array();
                                        vt.col(hetro_index*(total_Nbin+1)+k*phenocount+j) = temp_vec;
                                } 
                        }
        ...
        for (int k = 0; k < (total_Nbin+1); k++) {
                for (int j = 0; j < phenocount; j++) {
                        vt.col((total_Nbin+1)*total_Nbin*phenocount+k*phenocount+j) = wt.col(k*phenocount+j);
                }
        }
        Nbin = total_Nbin;
        ...
        cout<<"Number of individuals after filtering for phenotype : "<<mask.col(0).sum()<<endl;
        ...
        if (trace){
            ...
            trace_file.open(trpath.c_str(),  std::ofstream::out | std::ofstream::trunc);
        ...
        for (jack_index=0;jack_index<=Njack;jack_index++){
                ...
                for (int k = 0; k < phenocount; k++) {
                        ...
                        MatrixXdr herit_copy = herit;

                        for (int t = 0; t < Nbin; t++) {
                                float cur_herit = herit_copy(t, 0);
                                if (cur_herit < 0) {
                                        herit_copy(t, 0) = 0;
                                        herit_copy(Nbin, 0) += cur_herit;
                                }
                        }
                        ...
                        if (jack_index==Njack) {
                                ...
                                for (int m=0;m<(Nbin+1);m++) {
                                        for (int l=0;l<(Nbin+1);l++) {
                                                ...
                                                for (int t=0; t<(Nbin+1);t++) {
                                                        MatrixXdr z1=vt.col((t*(Nbin+1)*phenocount)+l*phenocount+k);
                                                        MatrixXdr z2=wt.col(m*phenocount+k);
                                                        double temp=(z1.array()*z2.array()).sum();
                                                        sum1+=herit_copy(t,0)*temp; 
                                ...
                                for (int i=0;i<(Nbin+1);i++) {
                                        ...
                                        asymp_se(i, k) = sqrt(cov_sigma(i, i));
                                } 
                                for (int i = 0; i < (Nbin+1); i++) {
                                        for (int j = 0; j < (Nbin+1); j++) {
                                                all_cov_mat(k, i*(Nbin+1)+j) = cov_sigma(i, j);
                                        }
                                } 
                                // cout << endl;
                                // cout << endl;   
                                MatrixXdr one_vec = MatrixXdr::Zero(Nbin+1, 1);
                                one_vec(1, 0) = 1;
                                if (gen_Nbin > 1) {
                                        for (int i=1; i<gen_Nbin-1;i++) {
                                                one_vec(i+1, 0) = 1;
                                                MatrixXdr sum_var = one_vec.transpose()*cov_sigma*one_vec;
                                                asymp_se(Nbin+i, k) = sqrt(sum_var(0, 0));
                                        }
                                }
	...
	enrich_g.resize(gen_Nbin,phenocount);
        ...
        for (int phen_index = 0; phen_index < phenocount; phen_index++) {
                ...
                for (int j=0;j<Nbin;j++){
                        if(j<gen_Nbin){
                                cout<<"Sigma^2_g["<<j<<"] : "<<point_est_preserved(j,phen_index)<<"  SE : "<<asymp_se(j,phen_index)<<endl;
                                outfile<<"Sigma^2_g["<<j<<"] : "<<point_est_preserved(j, phen_index)<<"  SE : "<<asymp_se(j,phen_index)<<endl;
                        }
                        else if (j<(gen_Nbin+nongen_Nbin)){
                                int k=j-gen_Nbin;
                                cout<<"Sigma^2_gxe["<<k<<"] : "<<point_est_preserved(j, phen_index)<<"  SE : "<<asymp_se(j,phen_index)<<endl;
                                outfile<<"Sigma^2_gxe["<<k<<"] : "<<point_est_preserved(j, phen_index)<<"  SE : "<<asymp_se(j,phen_index)<<endl;
                        }
                        else if (j<Nbin){
                                int k=j-gen_Nbin-nongen_Nbin;
                                cout<<"Sigma^2_nxe["<<k<<"] : "<<point_est_preserved(j, phen_index)<<"  SE : "<<asymp_se(j,phen_index)<<endl;
                                outfile<<"Sigma^2_nxe["<<k<<"] : "<<point_est_preserved(j, phen_index)<<"  SE : "<<asymp_se(j,phen_index)<<endl;
                ...
                //                 for (int m = 0; m <= j; m++) {
                //                         gene_sigma(j-1, 0) +=  point_est_preserved(m+1, phen_index); 
                //                 }
                //                 cout << "Sigma^2_g[1-"<< j+1 <<"] : " << gene_sigma(j-1, 0) << " SE : " << asymp_se(Nbin+j, phen_index) << endl;
                //                 outfile << "Sigma^2_g[1-"<< j+1 <<"] : " << gene_sigma(j-1, 0) << " SE : " << asymp_se(Nbin+j, phen_index) << endl;
                //         }
                ...
                if (bin_pheno_flag) {
                        for (int j=0;j<gen_Nbin; j++) {
                                cout<<"Sigma^2_g["<<j<<"] (liability model) : "<<point_est_preserved(j,phen_index)*liability_factor(phen_index,0)<<"  SE : "<<asymp_se(j,phen_index)*liability_factor(phen_index,0)<<endl;
                                outfile<<"Sigma^2_g["<<j<<"] (liability model) : "<<point_est_preserved(j,phen_index)*liability_factor(phen_index,0)<<"  SE : "<<asymp_se(j,phen_index)*liability_factor(phen_index,0)<<endl;
                        }

                        // if (gen_Nbin > 1) {
                        //         for (int j = 1; j < gen_Nbin-1; j++) {
                        //                 cout << "Sigma^2_g[1-"<< j+1 <<"] (liability model) : " << gene_sigma(j-1, 0)*liability_factor(phen_index,0) << " SE : " << asymp_se(Nbin+j, phen_index)*liability_factor(phen_index,0) << endl;
                        //                 outfile << "Sigma^2_g[1-"<< j+1 <<"] (liability model) : " << gene_sigma(j-1, 0)*liability_factor(phen_index,0) << " SE : " << asymp_se(Nbin+j, phen_index)*liability_factor(phen_index,0) << endl;
                        //         }
                ...
                float temp_gene_h2_se = 0;
                for (int j=0;j<Nbin;j++){
                        float current_asymp_se = point_est_adj_gxe(j,phen_index)/point_est_preserved(j, phen_index)*asymp_se(j, phen_index);
                        if(j<gen_Nbin){
                                if (j == 1) {temp_gene_h2_se =  current_asymp_se;}
                                cout<<"h2_g["<<j<<"] : "<<point_est_adj_gxe(j,phen_index)<<" SE : "<<current_asymp_se<<endl;
                                outfile<<"h2_g["<<j<<"] : "<<point_est_adj_gxe(j,phen_index)<<" SE : "<<current_asymp_se<<endl;
                                
                        }
                        else if (j<(gen_Nbin+nongen_Nbin)){
                                int k=j-gen_Nbin;
                                cout<<"h2_gxe["<<k<<"] : "<<point_est_adj_gxe(j,phen_index)<<" SE : "<<current_asymp_se<<endl;
                                outfile<<"h2_gxe["<<k<<"] : "<<point_est_adj_gxe(j,phen_index)<<" SE : "<<current_asymp_se<<endl;
                        }else if (j<Nbin){
                                int k=j-gen_Nbin-nongen_Nbin; 
                                cout<<"h2_nxe["<<k<<"] : "<<point_est_adj_gxe(j,phen_index)<<" SE : "<<current_asymp_se<<endl;
                                outfile<<"h2_nxe["<<k<<"] : "<<point_est_adj_gxe(j,phen_index)<<" SE : "<<current_asymp_se<<endl;
                        }
                        // simul_par(j, 5) = point_est_adj_gxe(j,phen_index); // h2g
                }

                MatrixXdr cov_sigma_2 = MatrixXdr::Zero(Nbin+1, Nbin+1);
                for (int i = 0; i < gen_Nbin; i++) {
                        float factor1 = point_est_adj_gxe(i,phen_index)/point_est_preserved(i, phen_index);
                        for (int j = 0; j < gen_Nbin; j++) {
                                float factor2 = point_est_adj_gxe(j,phen_index)/point_est_preserved(j, phen_index);
                                cov_sigma_2(i, j) = all_cov_mat(phen_index, i*(Nbin+1)+j)*factor1*factor2;
                        }
                }
                ...
                if (gen_Nbin > 1) {
                        if (gen_Nbin == 2) {
                                cout << "h2_g[gene] : " << point_est_adj_gxe(1, phen_index) << " SE : " << temp_gene_h2_se << endl;
                                outfile << "h2_g[gene] : " << point_est_adj_gxe(1, phen_index) << " SE : " << temp_gene_h2_se << endl; 
                        } else {
                                MatrixXdr one_vec_2 = MatrixXdr::Zero(Nbin+1, 1);
                                one_vec_2(1, 0) = 1;
                                for (int i=1; i<gen_Nbin-1;i++) {
                                        one_vec_2(i+1, 0) = 1;
                                        MatrixXdr sum_var = one_vec_2.transpose()*cov_sigma_2*one_vec_2;
                                        gene_h2_std(i-1, 0) = sqrt(sum_var(0, 0));
                                        for (int m = 0; m <= i; m++) {
                                                gene_h2(i-1, 0) +=  point_est_adj_gxe(m+1, phen_index); 
                                        }
                                        if (i == gen_Nbin-2) {
                                                cout << "h2_g[gene] : " << gene_h2(i-1, 0) << " SE : " << gene_h2_std(i-1, 0) << endl;
                                                outfile << "h2_g[gene] : " << gene_h2(i-1, 0) << " SE : " << gene_h2_std(i-1, 0) << endl;   
                                        }
                ...
                if (bin_pheno_flag) {
                        float current_asymp_se = 0;
                        for (int j=0;j<gen_Nbin; j++) {
                                current_asymp_se = point_est_adj_gxe(j,phen_index)/point_est_preserved(j, phen_index)*asymp_se(j, phen_index);
                                cout<<"h2_g["<<j<<"] (liability model) : "<<point_est_adj_gxe(j,phen_index)*liability_factor(phen_index,0)<<"  SE : "<<current_asymp_se*liability_factor(phen_index,0)<<endl;
                                outfile<<"h2_g["<<j<<"] (liability model) : "<<point_est_adj_gxe(j,phen_index)*liability_factor(phen_index,0)<<"  SE : "<<current_asymp_se*liability_factor(phen_index,0)<<endl;
                                if (gen_Nbin == 2) {
                                        cout<<"h2_g[gene] (liability model) : "<<point_est_adj_gxe(j,phen_index)*liability_factor(phen_index,0)<<"  SE : "<<current_asymp_se*liability_factor(phen_index,0)<<endl;
                                        outfile<<"h2_g[gene] (liability model) : "<<point_est_adj_gxe(j,phen_index)*liability_factor(phen_index,0)<<"  SE : "<<current_asymp_se*liability_factor(phen_index,0)<<endl;   
                                }
                        }

                        if (gen_Nbin > 2) {
                                // for (int j = 1; j < gen_Nbin-1; j++) {
                                cout << "h2_g[gene] (liability model) : " << gene_h2(gen_Nbin-2-1, 0)*liability_factor(phen_index,0) << " SE : " << gene_h2_std(gen_Nbin-2-1, 0)*liability_factor(phen_index,0) << endl;
                                outfile << "h2_g[gene] (liability model) : " << gene_h2(gen_Nbin-2-1, 0)*liability_factor(phen_index,0) << " SE : " << gene_h2_std(gen_Nbin-2-1, 0)*liability_factor(phen_index,0) << endl;
                                // }
                ...
                // cout << endl;
                // cout << "====== Jackknife results =====" << endl; 
                // outfile << endl;
                // outfile << "====== Jackknife results =====" << endl; 
                ...
                // if (trace) {
                //         string parpath=prefix + "." + std::to_string(phen_index) + ".par";
                //         std::ofstream par_file;
                //         par_file.open(parpath.c_str(),  std::ofstream::out | std::ofstream::trunc);
                //         par_file << "p_casual ld_ex maf_ex min_maf max_maf total_h2 num_simul" << endl;
                //         int num_par=7;
                //         int num_vc=Nbin+2;
                //         for (int j = 0; j < num_vc; j++) {
                //                 for (int k = 0; k < num_par-1; k++) {
                //                         par_file << simul_par(j, k) << " ";
                //                 }
                //                 par_file << simul_par(j, num_par-1) << endl;
                //         }
