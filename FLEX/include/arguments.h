...
void parse_args(int argc, char const *argv[]){
	...
	command_line_opts.perm_pheno_num=0;
	command_line_opts.bin_pheno_flag = false;
                ...
                */
				...
				//CHANGE(2/27)
                   ...
                   // CHANGE(03/04): makes more sense to have flags as a boolean true. TODO: also, why aren't we using getopt_long?
					...
					command_line_opts.perm_pheno_num = atoi(argv[i+1]);
					i++;
				} else if ((strcmp(argv[i], "-b")==0) || (strcmp(argv[i], "--binary")==0)) {
					command_line_opts.bin_pheno_flag = true;
				}
							//CHANGE(11/12)
