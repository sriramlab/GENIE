...
//                      temp(i,0)=1;  
        ...
        cout<<Enviro.rows()<<"   "<<Enviro.cols()<<endl;
        cout<<covariate.rows()<<" "<<covariate.cols()<<endl;
        cout<<covNum<<" "<<Nenv<<endl;
         if(add_env_to_cov==true){
...
int T_Nbin=Nbin+nongen_Nbin+Nenv;
...
for (int env_index=0;env_index<Nenv;env_index++){
                ...
                //         hetro_all_zb=all_Uzb.array().colwise()*Enviro.col(env_index).array();
                //         hetro_all_zb=hetro_all_zb.array().colwise()*Enviro.col(env_index).array();
                  ...
                  //
len.push_back(1);
//len.push_back(mask.sum()); /// adjust denominator of sigma^2_{NxE}
}
...
cout<<endl<<"Heritabilities(Adjusted for GxE): "<<endl;
outfile<<"Heritabilities(Adjusted for GxE): "<<endl;
for (int j=0;j<T_Nbin;j++){
     if(j<Nbin){
     cout<<"h^2_adj_g"<<j<<" : "<<point_est_adj_gxe(j,0)<<" SE: "<<SEjack_adj_gxe(j,0)<<endl;
     outfile<<"h^2_adj_g"<<j<<" : "<<point_est_adj_gxe(j,0)<<" SE: "<<SEjack_adj_gxe(j,0)<<endl;
     }
     else{
     int k=j-Nbin;
     cout<<"h^2_adj_ge"<<k<<" : "<<point_est_adj_gxe(j,0)<<" SE: "<<SEjack_adj_gxe(j,0)<<endl;
     outfile<<"h^2_adj_ge"<<k<<" : "<<point_est_adj_gxe(j,0)<<" SE: "<<SEjack_adj_gxe(j,0)<<endl;
     }
}
cout<<"Total h^2_adj : "<<point_est_adj_gxe(T_Nbin,0)<<" SE: "<<SEjack_adj_gxe(T_Nbin,0)<<endl;
outfile<<"Total h^2_adj : "<<point_est_adj_gxe(T_Nbin,0)<<" SE: "<<SEjack_adj_gxe(T_Nbin,0)<<endl;
cout<<"Total h^2_adj_GxE : "<<point_est_adj_gxe(T_Nbin+1,0)<<" SE: "<<SEjack_adj_gxe(T_Nbin+1,0)<<endl;
outfile<<"Total h^2_adj_GxE : "<<point_est_adj_gxe(T_Nbin+1,0)<<" SE: "<<SEjack_adj_gxe(T_Nbin+1,0)<<endl;
cout<<"Total h^2_adj_G : "<<point_est_adj_gxe(T_Nbin+2,0)<<" SE: "<<SEjack_adj_gxe(T_Nbin+2,0)<<endl;
outfile<<"Total h^2_adj_G : "<<point_est_adj_gxe(T_Nbin+2,0)<<" SE: "<<SEjack_adj_gxe(T_Nbin+2,0)<<endl;
...
for(int i=0;i<Nsnp;i++){
   ...
   else 
   {
        ...
        for(int j=0;j<Nbin;j++){
                ...
                if(annot_bool[i][Nbin+j]==1 && temp!=k)
...
for (int j=0;j<Nbin;j++){
        int k=j+Nbin;
        cout<<"h^2_ge"<<j<<" : "<<point_her_cat_ldsc(k,0)<<" SE: "<<se_her_cat_ldsc(k,0)<<endl;
        outfile<<"h^2_ge"<<j<<" : "<<point_her_cat_ldsc(k,0)<<" SE: "<<se_her_cat_ldsc(k,0)<<endl;
...
for (int j=0;j<Nbin;j++){
        cout<<"Enrichment of G bin "<<j<<" : "<<point_her_cat_ldsc(j,0)<<" SE: "<<se_her_cat_ldsc(j,0)<<endl;
        outfile<<"Enrichment of G bin "<<j<<" : "<<point_her_cat_ldsc(j,0)<<" SE: "<<se_her_cat_ldsc(j,0)<<endl;
}
for (int j=0;j<Nbin;j++){
        int k=j+Nbin;
         cout<<"Enrichment of GxE bin "<<j<<" : "<<point_her_cat_ldsc(k,0)<<" SE: "<<se_her_cat_ldsc(k,0)<<endl;
        outfile<<"Enrichment of GxE bin "<<j<<" : "<<point_her_cat_ldsc(k,0)<<" SE: "<<se_her_cat_ldsc(k,0)<<endl;
}
