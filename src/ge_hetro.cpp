...
X1.resize(3,3);
Y1.resize(3,1);
X1<<290430,7.28383e+12,138436,7.28383e+12, 2.81266e+21,4.9188e+12,138436,4.9188e+12,138557;
Y1<<158305,4.71974e+12,283618;
cout<<"LHS"<<endl<<X1<<endl;
cout<<"RHS"<<endl<<Y1<<endl;

//MatrixXdr herit1=X1.colPivHouseholderQr().solve(Y1);

MatrixXdr herit1=X1.ldlt().solve(Y1);
cout<<"solution"<<endl<<herit1<<endl;
double relative_error = (X1*herit1 - Y1).norm() / Y1.norm(); // norm() is L2 norm
...
yXXy=MatrixXdr::Zero(Nbin+nongen_Nbin+Nenv,Njack+1);
...
if(both_side_cov==true)
	...
	yy=(pheno.array()*pheno.array()).sum();
...
//point_est_adj_gxe.resize(Nbin+3,1);
//jack_adj_gxe.resize(Nbin+3,Njack);
//
...
point_est_adj_gxe(Nbin,0)=temp_sig;
temp_sig=0;
if(Annot_x_E==true){
for(int k=gen_Nbin;k<(2*gen_Nbin);k++)
	temp_sig+=point_est_adj_gxe(k,0);
point_est_adj_gxe(Nbin+1,0)=temp_sig;// total GxE
}else{
point_est_adj_gxe(Nbin+1,0)=point_est_adj_gxe(gen_Nbin,0);

}
temp_sig=0;
for(int k=0;k<gen_Nbin;k++)
        temp_sig+=point_est_adj_gxe(k,0);
point_est_adj_gxe(Nbin+2,0)=temp_sig;//total G
...
for (int i=0;i<Njack;i++){
   ...
   jack_adj_gxe(Nbin,i)=temp_sig;
   temp2_sig=0;
   if(Annot_x_E==true){
   for(int k=gen_Nbin;k<(2*gen_Nbin);k++)
	   temp2_sig+=jack_adj_gxe(k,i);
   jack_adj_gxe(Nbin+1,i)=temp2_sig;
   }else{
     jack_adj_gxe(Nbin+1,i)=jack_adj_gxe(gen_Nbin,i);
   }
   temp2_sig=0;
   for(int k=0;k<gen_Nbin;k++)
           temp2_sig+=jack_adj_gxe(k,i);
   jack_adj_gxe(Nbin+2,i)=temp2_sig;
